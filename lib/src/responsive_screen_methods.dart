// context from init.
// dimensions you need to design for. give us base design
// get height/width
// ask ibtihaaj about the name he suggested

abstract class ResponsiveScreenMethods {
  double getMyDynamicFontSize(double figmaFontSize, {double maxlimit});
  double getMyDynamicWidth(double figmaWidth, {double maxlimit});
  double getMyDynamicHeight(double figmaHeight, {double maxlimit});
  double getBottomMarginforBigScreens();
}
